...
bool WSProtocol::WSShakeHand(Session* s, char* body, int len) 
{
	...
	if (has_sec_key && is_shaker_ended) 
	{
		...
		s->SendData((unsigned char*)send_client, strlen(send_client));
		return true;
	}
	return false;
}
...
bool
WSProtocol::ReadWSHeader(unsigned char* recv_data, int recv_len, int* pkg_size, int* out_header_size) 
{
...
void
WSProtocol::ParserWSRecvData(unsigned char* raw_data, unsigned char* mask, int raw_len) 
{
...
unsigned char*
WSProtocol::PackageWSSendData(const unsigned char* raw_data, int len, int* ws_data_len) 
{
	int head_size = 2;
	...
	// cache malloc
	unsigned char* data_buf = (unsigned char*)MyCacheAlloc::CacheAlloc(wbuf_allocer,head_size+ len);
	data_buf[0] = 0x81;
	if (len <= 125) 
	{
		data_buf[1] = len;
...
void
WSProtocol::FreeWSSendPkg(unsigned char* ws_pkg) 
{
	// cache free
	MyCacheAlloc::CacheFree(wbuf_allocer, ws_pkg);
}